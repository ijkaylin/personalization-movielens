...
class KNN:
    def __init__(self, df, complete_set, similarity = 'cosine', by = 'item', k = 5):
        ...
        row_ids = self.complete_set.copy()[self.row].unique()
        col_ids = self.complete_set.copy()[self.col].unique()

        self.rowix_id_map = dict(zip(range(len(row_ids)), row_ids))
        self.colix_id_map = dict(zip(range(len(col_ids)), col_ids))

        self.rowid_ix_map = { v: k for k, v in self.rowix_id_map.copy().iteritems() }
        self.colid_ix_map = { v: k for k, v in self.colix_id_map.copy().iteritems() }

        # change all user/movie ids, need to undo this when predicting
        self.df['OriginalRow'] = self.df.copy()[self.row]
        self.df['OriginalCol'] = self.df.copy()[self.col]
...
def custom_knn(train, complete, similarity, by, k):
    _train = train.copy()
